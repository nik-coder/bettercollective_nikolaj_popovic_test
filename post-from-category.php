 ...
 * Author URI: https://wpmanesh.com/
 * License: GNU/GPL http://www.gnu.org/copyleft/gpl.html
 */

define( 'PFC_DIR', rtrim( plugin_dir_path( __FILE__ ), '/' ) );

require_once PFC_DIR . '/functions.php';
require_once PFC_DIR . '/admin.php';
...
class PFCWidget extends WP_Widget {
	...
	*/	
	...
	function pfc_load_frontend_scripts() {
		...
		wp_enqueue_script( 'pfc-custom', rtrim( plugin_dir_url( __FILE__ ), '/' ) . '/assets/pfc-custom.js', array( 'jquery' ), '5.0.1' );
	}
	...
	*/
	...
	function dropdown_post_order_by( $args ) {
		...
		//<option value="">' . attribute_escape(__("Select Post")) . '</option>' .
		//wp_get_archives("type=daily") . '</select>';

		//$posts = get_posts(array('numberposts'=>-1)); //Get all published posts
		//foreach ($posts as $post){
			//echo '<form method="GET">
    			//<select name="orderby" id="orderby">
        			//<option value="date">' . get_the_time('Y-m-d', $post->ID) . '</option>
				//</select>
			//<form>';

			//$output .= get_the_time('Y-m-d', $post->ID); //Echos date in Y-m-d format.
		//}
